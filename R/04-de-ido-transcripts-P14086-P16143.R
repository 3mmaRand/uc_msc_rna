...
library("DESeq2")
library("ggrepel")
# high IDO transcripts P14086 and P16143 and the other donors?
...
###############################################################################
# Volcano data for Differential expression analysis for (P14086 and P16143)
# vs other donors:
###############################################################################
...
de_results_idotrans_vol <- de_results_idotrans %>%
  ...
  filter(!is.na(padj))
# dim(de_results_idotrans_vol) 8622
# length(unique(de_results_idotrans_vol$gene_id)) 8622
...
annotations_idotrans <- AnnotationDbi::select(org.Hs.eg.db::org.Hs.eg.db,
                                     ...
                                     columns = c("SYMBOL",
                                                 ...
                                                 "GENETYPE"),
                                     keytype = "ENSEMBL")
# dim(annotations_idotrans) 8665    we have 43 duplicates
...
write_csv(de_results_idotrans_vol,
          file = "data-processed/de_ido_resp_idotrans_results_P14086_P16143.csv")
...
write_csv(de_results_sig_up_idotrans_high,
          file = "data-processed/diff-expr-by-idotrans-sig-up-high_P14086_P16143.csv")
...
write_csv(de_results_sig_down_idotrans_high,
          file = "data-processed/diff-expr-by-idotrans-sig-down-high_P14086_P16143.csv")
...
de_results_idotrans_vol  %>%
  ...
  annotate("text", x = -5,  y = 4.5,
           label = "Down regulated in P14086 + P16143 \nFDR < 0.05, log2FC < 0",
           hjust = 0) +
  annotate("text", x = 5,  y = 4.5,
           label = "Up regulated in P14086 + P16143 \nFDR < 0.05, log2FC > 0",
           hjust = 1) +
  ...
  guides(colour = "none") +
  ggtitle("Volcano plot comparing high IDO transcripts (P14086 and P16143) and the others",
          subtitle = "Pink = fdr < 0.05") +
  theme(panel.background = element_rect(fill = "white"),
        panel.border = element_rect(colour = "black",
                                    fill = NA))

ggsave(filename = "reports/figures/volcano-idotrans_P14086_P16143.png",
       units = "in",
       height = 8,
       width = 10,
       dpi = 300,
